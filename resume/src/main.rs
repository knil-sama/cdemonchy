...
pub use crate::yaml_struct::{
    AdditionalLink, Config, ContentDetail, ContentWrapper, LayoutDetail, Sass,
};
...
fn generate_yaml_config_from_xml_resume(xml_resume: Resume) {
        ...
        content: xml_resume
            ...
            .map(|project| ContentDetail {
                ...
                link_text: project
                    .website
                    .as_ref()
                    .map(|website| website.link_text.clone()),
    ...
    let content_education: ContentWrapper = ContentWrapper {
        ...
        content: vec![ContentDetail {
            ...
            caption: Some(format!(
                "{} - {}",
                xml_resume.education.start_date.format("%Y"),
                xml_resume.education.end_date.format("%Y")
            )),
    ...
    let content: Vec<ContentWrapper> =
        vec![content_projects, content_experiences, content_education];
    let yaml_config: Config = Config {
        repository: "knil-sama/cdemonchy".to_string(),
        version: 2,
        name: format!(
            "{} {}",
            xml_resume.personal.firstname, xml_resume.personal.lastname
        ),
...
fn generate_latex_file(xml_resume: Resume, _rendered_template_filepath: String) {
    let latex_resume = LatexResume {
        ...
        objective: format_line_break(
            &highlight_skill(&xml_resume.about_me, &TypeHighlight::Latex),
            &TypeHighlight::Latex,
        ),
        experiences: xml_resume
            .experiences
            .into_iter()
            .map(|experience| askama_struct::Experience {
                ...
                description: format_line_break(
                    &highlight_skill(&experience.description, &TypeHighlight::Latex),
                    &TypeHighlight::Latex,
                ),
...
async fn malt_update() -> WebDriverResult<()> {
    let config: TomlConfig =
        toml::from_str(&std::fs::read_to_string("src/.secrets.toml").unwrap()).unwrap();
    ...
    println!(
        "{:?}",
        driver
            .find(By::ClassName("profile-headline-read-fullname"))
            .await?
            .value()
            .await
            .unwrap()
